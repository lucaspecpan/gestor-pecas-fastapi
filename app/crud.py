# File: app/crud.py (Versão 5.8 - Corrigido Erros Pylance)
...
from barcode import EAN13 # <-- Import necessário para generate_ean13
# from barcode.writer import ImageWriter # Não usado diretamente no CRUD

# Importa de forma absoluta a partir da raiz do pacote 'app'
from app import models, schemas, config

# --- Função Auxiliar EAN13 (Adicionada aqui) ---
def generate_ean13(internal_id: int) -> Optional[str]:
    """Gera um EAN-13 'interno' usando o ID da peça/variação."""
    if not internal_id: return None
    try:
        # Usa prefixo 290 + ID zerofill até 12 dígitos
        base_number_str = f"290{internal_id:09d}"[:12]
        ean = EAN13(base_number_str)
        return ean.ean13 # Retorna a string do EAN-13 gerado
    except Exception as e:
        # Logar o erro é importante em produção
        print(f"AVISO: Não foi possível gerar EAN-13 para ID {internal_id}: {e}")
        return None
...
def create_montadora(db: Session, montadora: schemas.MontadoraCreate) -> models.Montadora:
    nome_upper = montadora.nome_montadora # Schema já deve ter feito upper
    if get_montadora_by_name(db, nome_montadora=nome_upper):
        raise ValueError(f"Montadora '{nome_upper}' já existe.")
    ...
    # Correção de indentação aqui (se necessário, garantir 4 espaços)
    try:
        db_montadora = models.Montadora(cod_montadora=next_cod, nome_montadora=nome_upper)
...
def get_modelo_by_nome_and_montadora(db: Session, nome_modelo: str, cod_montadora: int) -> Optional[models.ModeloVeiculo]:
    return db.query(models.ModeloVeiculo)\
             .filter(models.ModeloVeiculo.cod_montadora == cod_montadora,
                     func.upper(models.ModeloVeiculo.nome_modelo) == nome_modelo.upper())\
             .first()
def get_next_modelo_sequencial(db: Session, cod_montadora: int) -> int:
    max_seq = db.query(func.max(models.ModeloVeiculo.cod_sequencial_modelo))\
                .filter(models.ModeloVeiculo.cod_montadora == cod_montadora)\
                .scalar()
...
def get_or_create_modelo(db: Session, nome_modelo: str, cod_montadora: int) -> models.ModeloVeiculo:
    ...
    if not nome_upper: raise ValueError("Nome do modelo vazio.")
    ...
    db_modelo = get_modelo_by_nome_and_montadora(db, nome_modelo=nome_upper, cod_montadora=cod_montadora)
    if db_modelo: return db_modelo
    else:
        # Correção de indentação aqui (se necessário, garantir 4 espaços)
        try:
            next_seq = get_next_modelo_sequencial(db, cod_montadora=cod_montadora)
            novo_modelo = models.ModeloVeiculo( cod_montadora=cod_montadora, nome_modelo=nome_upper, cod_sequencial_modelo=next_seq )
            db.add(novo_modelo); db.commit(); db.refresh(novo_modelo)
            print(f"Novo modelo: {novo_modelo.nome_modelo} (Seq: {novo_modelo.cod_sequencial_modelo}) p/ Mont {cod_montadora}")
            return novo_modelo
        except exc.SQLAlchemyError as e:
            db.rollback(); print(f"Erro DB criar modelo {nome_upper}: {e}")
            db_modelo_retry = get_modelo_by_nome_and_montadora(db, nome_modelo=nome_upper, cod_montadora=cod_montadora)
            if db_modelo_retry: return db_modelo_retry
...
def get_next_cod_final_item(db: Session, cod_montadora: int, modelo_id: int, nome_item: str) -> int:
    ...
    if not nome_item_upper: raise ValueError("Nome do Item vazio.")
    try:
        min_cod_item = db.query(func.min(models.Peca.cod_final_item))\
                         .filter(models.Peca.cod_montadora == cod_montadora,
                                 models.Peca.cod_modelo == modelo_id,
                                 func.upper(models.Peca.nome_item) == nome_item_upper)\
                         .scalar()
        if min_cod_item is None: return 999
        elif min_cod_item <= 0: return -1
        else: return min_cod_item - 1
...
def search_pecas_crud(db: Session, search_term: str, skip: int = 0, limit: int = 50) -> List[models.Peca]:
    ...
    try:
        query = db.query(models.Peca).join(models.Peca.montadora_rel).join(models.Peca.modelo_rel)\
...
def create_peca_variacao(db: Session, peca_data: schemas.PecaCreate, image_urls: List[str] = []) -> models.Peca:
    ...
    cod_modelo_id = db_modelo.id; cod_sequencial_modelo = db_modelo.cod_sequencial_modelo
    next_fff = get_next_cod_final_item(db, peca_data.cod_montadora, cod_modelo_id, peca_data.nome_item)
    if next_fff < 0: raise ValueError(f"Limite/Erro ({next_fff}) cód item p/ M{peca_data.cod_montadora}/Mod{cod_sequencial_modelo:02d}/{peca_data.nome_item}.")
    codigo_base_sku = f"{peca_data.cod_montadora:03d}{cod_sequencial_modelo:02d}{next_fff:03d}"
    ...
    for key in ['nome_item', 'descricao_peca', 'codigo_oem', 'anos_aplicacao', 'posicao_porta']:
    ...
    try:
        ...
        ean13 = generate_ean13(peca_id) # Função generate_ean13 agora está neste arquivo
...
def update_peca_variacao(db: Session, peca_id: int, peca_update_data: schemas.PecaBase) -> Optional[models.Peca]:
    ...
    update_data = peca_update_data.model_dump(exclude_unset=True, exclude={'tipo_variacao', 'cod_modelo', 'nome_item'})
...
def delete_peca_variacao(db: Session, peca_id: int) -> bool:
    ...
    except exc.IntegrityError as e: db.rollback(); print(f"Erro Integridade deletar peça {peca_id}: {e}"); raise ValueError(f"Erro de integridade: {e}")
    except exc.SQLAlchemyError as e: db.rollback(); print(f"Erro SQLA deletar peça {peca_id}: {e}"); raise ValueError("Erro interno deletar.")
