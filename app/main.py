...
async def handle_add_peca(
    ...
    descricao_peca: Optional[str] = Form(None), categoria: Optional[str] = Form(None),
    codigo_oem: Optional[str] = Form(None), anos_aplicacao: Optional[str] = Form(None),
    posicao_porta: Optional[str] = Form(None), quantidade_estoque: int = Form(..., ge=0),
    preco_venda: float = Form(..., ge=0), custo_ultima_compra: Optional[float] = Form(0.0, ge=0),
    aliquota_imposto_percent: Optional[float] = Form(0.0, ge=0, le=100), custo_estimado_adicional: Optional[float] = Form(0.0, ge=0),
    data_ultima_compra: Optional[date] = Form(None), imagens: List[UploadFile] = File([], alias="imagens[]", max_items=10),
    db: Session = Depends(get_db) ):

    error_msg = None; success_msg = None; uploaded_image_urls = []
    if len(imagens) > 10: error_msg = "Erro: Máx 10 imagens."; return templates.TemplateResponse(request=request, name="partials/messages.html", context={"error_message": error_msg})
    ...
    try:
        peca_schema_data = {
            ...
            "categoria": categoria, "codigo_oem": codigo_oem, "anos_aplicacao": anos_aplicacao,
